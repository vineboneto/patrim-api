...
import { LoadOwnersRepositorySpy, mockLoadOwnersRepositoryParams } from '@/tests/data/mocks'
...
describe('DbLoadOwners', () => {
  test('Should call LoadOwnersRepository with correct value', async () => {
    ...
    const params = mockLoadOwnersRepositoryParams()
  ...
  test('Should return empty array if LoadOwnersRepository returns empty array', async () => {
    const { sut, loadOwnersRepositorySpy } = makeSut()
    loadOwnersRepositorySpy.models = []
    const data = await sut.load(mockLoadOwnersRepositoryParams())
    expect(data).toEqual([])
  })
  ...
  test('Should return owners on success', async () => {
    const { sut, loadOwnersRepositorySpy } = makeSut()
    const data = await sut.load(mockLoadOwnersRepositoryParams())
    expect(data).toEqual(loadOwnersRepositorySpy.models)
  })
  ...
  test('Should throw if LoadOwnersRepository throws', async () => {
    ...
    const promise = sut.load(mockLoadOwnersRepositoryParams())
